...
/**
 * 是否有媒体编辑面板打开中（标注 / 裁剪 / 选帧 / 片段截取 / 音频片段截取 / 打光）。
...
export function isMediaEditorOpen(): boolean {
  ...
  return !!(s.annotatingNodeId || s.croppingNodeId || s.frameCaptureNodeId || s.clipCaptureNodeId || s.audioClipNodeId || s.lightingNodeId);
}
