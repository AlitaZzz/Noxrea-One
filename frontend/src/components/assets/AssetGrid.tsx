...
export default function AssetGrid({ assets, folders, folderCounts, selectedIds, onToggleSelect, onInsertCanvas, onRename, onDelete, onEnterFolder }: Props) {
  ...
  return (
    <div className="grid gap-3 pb-2" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))" }}>
      {/* Folders first */}
      {folders?.map((folder) => (
        <FolderCard key={folder.id} folder={folder} count={folderCounts?.[folder.id] || 0} onClick={onEnterFolder || (() => {})} />
      ))}
      {/* Then assets */}
      {assets.map((asset) => (
        <AssetCard
