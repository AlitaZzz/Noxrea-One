...
interface Props {
  open: boolean;
  onClose: () => void;
  onCreate: (inputs: CreateAssetInput[]) => void;
  folders?: AssetFolder[];
}
...
export default function AssetCreateDialog({ open, onClose, onCreate, folders }: Props) {
  ...
  async function processEntry(
    ...
    const [, uploadResult] = await Promise.all([
      ...
      // Task 2: 上传原文件
      new Promise<{ url: string | null }>(async (resolve) => {
        try {
          onUpdate(id, { status: "uploading" });
          const url = await uploadFile(file, (pct: number) => {
    ...
    if (uploadResult.url) {
      ...
      if (isVideo(file) && uploadResult.url.includes("/api/files/")) {
        (async () => {
          try {
            ...
            const res = await fetch(`${BASE}/api/files/capture-frame`, {
              ...
              body: JSON.stringify({ url: uploadResult.url, time: 0.1 }),
            });
            if (res.ok) {
