...
const TOOL_GUIDANCE = {
  ...
  content:
    ...
    "3. create_node 支持一次批量创建（≤6 个），同批次连线可用序号（\"1\"）引用同批节点。\n" +
    "4. 布局交给 arrange_canvas，不要手动计算节点坐标；需要展示某节点时用 set_viewport 或 select_nodes。\n" +
    "5. 一轮内如需多个工具调用，按依赖顺序逐轮发起：先创建拿到 id，再连线 / 更新。\n" +
    "6. 全部操作完成后调用 message_user 简要总结（创建/修改了什么、节点 id、建议），然后停止。\n" +
