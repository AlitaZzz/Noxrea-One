...
export async function createFrameSprite(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      ffmpeg.kill("SIGKILL");
      logEvent("media", {
        stage: "frame_sprite_timeout",
        video: path.basename(videoPath),
        timeoutMs: FFMPEG_SPRITE_TIMEOUT_MS,
      });
      reject(new Error(`ffmpeg timed out after ${FFMPEG_SPRITE_TIMEOUT_MS}ms`));
    ...
    ffmpeg.on("error", (err) => {
      logEvent("media", {
        stage: "frame_sprite_spawn_failed",
        video: path.basename(videoPath),
        error: err.message,
        ffmpegBin,
      });
...
export async function extractVideoClip(
  ...
  if (!stat || stat.size === 0) {
    logEvent("media", { stage: "clip_extract_empty", video: path.basename(videoPath) });
...
export async function cropVideoRegion(
  ...
  const stat = await fs.stat(outputPath).catch(() => null);
  if (!stat || stat.size === 0) {
    logEvent("media", { stage: "crop_video_empty", video: path.basename(videoPath) });
    throw new Error("Video crop produced an empty file");
  }

  logEvent("media", { stage: "crop_video", video: path.basename(videoPath) });
