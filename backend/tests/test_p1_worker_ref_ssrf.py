...
- 路径穿越 → 拒绝
- bg_removal 非同源源图 → 返回 (None, "must be hosted on this service")
...
重构后位置：
- 参考图/同源解析：app.services.resolvers.reference
- 背景移除推理封装：app.services.inference.bg_removal（返回 (url, error) 元组，状态由 executor 处理）
- 单任务执行：app.services.worker.executor
"""
import base64
...
import app.services.resolvers.reference as reference
import app.services.inference.bg_removal as bg_removal
import app.services.worker.executor as executor


# ── resolve_refs / read_self_file ────────────────────────────────
...
async def test_resolve_refs_self_url_no_network(tmp_path, monkeypatch):
    ...
    # 把 reference.httpx 整块替换成 MagicMock：若同源分支仍依赖网络，测试会直接崩
    monkeypatch.setattr(reference, "httpx", MagicMock())

    rel = "http://testserver/api/files/7/ab/deadbeef.png"
    out = await reference.resolve_refs([rel], uid)
...
def test_read_self_file_cross_user_rejected(tmp_path, monkeypatch):
    ...
    # 他人 uid → None（不读盘）
    assert reference.read_self_file("http://testserver/api/files/8/ab/deadbeef.png", user_id=7) is None
    # 自身 uid → (bytes, mime)
    pair = reference.read_self_file("http://testserver/api/files/7/ab/deadbeef.png", user_id=7)
...
async def test_resolve_refs_blocks_internal(tmp_path, monkeypatch):
    ...
    for bad in ["http://127.0.0.1:6379/x", "http://169.254.169.254/latest/meta-data/"]:
        out = await reference.resolve_refs([bad], user_id=7)
        assert out == [bad], out
...
def test_read_self_file_path_traversal(tmp_path, monkeypatch):
    ...
    assert reference.read_self_file(rel, user_id=7) is None


# ── inference.bg_removal.process ───────────────────────────────────
...
async def test_bg_removal_rejects_external_source(monkeypatch):
    ...
    url, err = await bg_removal.process(task)
    assert url is None
    assert "must be hosted" in (err or "")
...
async def test_bg_removal_self_source_reads_disk(monkeypatch):
    ...
    monkeypatch.setattr(bg_removal.httpx, "AsyncClient", FakeClient)
    monkeypatch.setattr(bg_removal, "save_upload_bytes",
                        AsyncMock(return_value="http://testserver/api/files/7/result.png"))

    url, err = await bg_removal.process(task)
    assert url == "http://testserver/api/files/7/result.png"
    assert err is None
...
# ── executor.process_task base_url 校验 ────────────────────────────
